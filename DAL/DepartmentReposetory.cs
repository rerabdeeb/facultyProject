     ...
     public class DepartmentReposetory:IDepartmentReposetory
    {
        ...
        public bool Update(DepartmentsViewModel dept)
        {
            try {
                ...
                Dept obj = context.Dept.FirstOrDefault(x => x.ID == dept.ID);
                if (obj != null)
                {
                    obj.DeptName = dept.DeptName;
                    context.SaveChanges();
        ...
        public bool Delete(int id)
        {
            ...
            try
            {
                if (id > 0)
                {
                    Dept obj = context.Dept.FirstOrDefault(x => x.ID == id);

                    if (obj != null)
                    {
                        context.Dept.Remove(obj);
                        context.SaveChanges();
                        return true;
        ...
        {
            string deptName = context.Dept.Where(x => x.ID == id).Select(x => x.DeptName).FirstOrDefault();
        ...
        {
            Dept dept = context.Dept.FirstOrDefault(x => x.ID == id);
            DepartmentsViewModel obj = new DepartmentsViewModel();
            obj.ID = dept.ID;
            obj.DeptName = dept.DeptName;
